...
class Test_BaseDataSource:
    ...
    def test__sync_data_missing_values(self):
        index_a = [1., 10., 20., 30., 40., 50.]
        values_a = [1.0] * len(index_a)
        series_a = pd.Series(data=values_a, index=index_a)
        ...
        df_expect = pd.DataFrame(
            data={
                "a": [np.nan, 1.0, 1.0, 1.0, np.nan, 1.0, 1.0, 1.0, np.nan],
                "b": [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, 2.0],
                "c": [3.0, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan],
                "d": [np.nan, np.nan, np.nan, np.nan, 4.0, np.nan, np.nan, np.nan, np.nan],
            },
            index=[-100., 1., 10., 20., 25., 30., 40., 50., 100.]
        )

        pd.testing.assert_frame_equal(df_out, df_expect)

    def test__sync_data_string_values(self):
        index_a = [1., 20., 60.]
        values_a = ["value_a1", "value_a2", "value_a3"]
        series_a = pd.Series(data=values_a, index=index_a)
        ...
        df_expect = pd.DataFrame(
            data={
                "a": [np.nan, "value_a1", "value_a2", np.nan, "value_a3", np.nan],
                "b": [np.nan, np.nan, np.nan, np.nan, np.nan, "value_b1"],
                "c": ["value_c1", np.nan, np.nan, np.nan, np.nan, np.nan],
                "d": [np.nan, np.nan, np.nan, "value_d1", np.nan, np.nan],
            },
            index=[-100., 1., 20., 25., 60., 100.]
        )
